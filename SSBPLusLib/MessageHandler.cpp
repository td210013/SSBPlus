...
void CMessageHandler::OnRecieveMessage(CPipeServer * pPipeServer, IGTGenObjPtr pGenObjPtr)
{
    ...
    try
    {
        ...
        if (_tcsicmp((_TCHAR*)str, _T("config")) == 0)
        {
            ...
            CString csName = CString((LPCTSTR)m_ptrGenOb->GetAttribute(_T("/"), _T("_name")));

            if (csName.CompareNoCase(_T("pa")) == 0)
            {
                Trace(TRACE_INFO, _T("Received PA Configuration Message"));
                PostStatusMsgToParent(_T("Received PA Configuration Message"));
                OnPaConfigMsg(pPipeServer, m_ptrGenOb);
            }
            else
            {
                Trace(TRACE_INFO, _T("Received PLA Configuration Message"));
                PostStatusMsgToParent(_T("Received PLA Configuration Message"));
                OnPlaConfigMsg(pPipeServer, m_ptrGenOb);
            }
        }
...
void CMessageHandler::OnPaConfigMsg(CPipeServer * pPipeServer, IGTGenObjPtr pGenObjPtr)
{
    try
    {
        if (pGenObjPtr != NULL)
...
void CMessageHandler::OnPlaConfigMsg(CPipeServer * pPipeServer, IGTGenObjPtr pGenObjPtr)
{
    /*
    try
    {
