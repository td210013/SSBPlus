#include "stdafx.h"
#include "CPlaConfigMsg.h"
#include "Logging.h"


CCPlaConfigMsg::CCPlaConfigMsg() : 
    hdr(L""),
    nNoOfitems(0),
    bEnable(false),
...
void CCPlaConfigMsg::Set(IGTGenObjPtr a_ptrGenObj)
{
    try
    {
        if (a_ptrGenObj != NULL)
        {
            ...
            if (csName.CompareNoCase(_T("PLA")) == 0)
            {
                hdr = a_ptrGenObj->GetXml(_T("/hdr"));

                CString csTimeout = (LPCTSTR)GetObjectValue(_T("timeout"));
                CString csCenterDetection = (LPCTSTR)GetObjectValue(_T("centerDetection"));
                CString csEnable = (LPCTSTR)GetObjectValue(_T("enable"));
                CString csValidation = (LPCTSTR)GetObjectValue(_T("validation"));
                CString csNumofItems = (LPCTSTR)GetObjectValue(_T("numberofItems"));
                CString csScaleMetric = (LPCTSTR)GetObjectValue(_T("scalemetric"));
...
IGTGenObjPtr CCPlaConfigMsg::Get()
{
    HRESULT hr = S_OK;
...
_bstr_t CCPlaConfigMsg::GetObjectValue(IGTGenObjPtr a_ptrGenObj, LPWSTR a_szName)
{
    _variant_t var;
    _bstr_t str = _T("");
...
BOOL CCPlaConfigMsg::SetObjectValue(IGTGenObjPtr a_ptrGenObj, LPWSTR a_szName, _bstr_t a_szValue)
{
    a_ptrGenObj->SetValue(a_szName, a_szValue);
    return 0;
}

const std::string CCPlaConfigMsg::GetCurrentDateTime()
{
    SYSTEMTIME st;
