...
NcrZebraCamera * CGTOControlObject::m_pNcrZebra = NULL;
...
BOOL CGTOControlObject::InitializeGTOCO(CWnd& a_CWnd, NcrZebraCamera * a_pNcrZebra)
{
	BOOL bResult = TRUE;
	try
	{
...
void CGTOControlObject::handlegetMatchingItems()
{
	try
	{
		if(m_pGenObj != NULL)
		{
            ...
            bool bSuccess = m_pNcrZebra->TakeSnapShot(image_data);
...
{   
    ...
    // test B64 conversions 
    //CBase64EncodeDecode oB64;

    //std::vector<uint8_t> vDecoded;

    //oB64.Base64Decode((const void*)a_image_data.data, a_image_data.size, vDecoded);
