...
#include "Resource.h"
#include "map"
#include "RPC.H"  // for UuidCreate etc.
...
std::vector<int> CGTOControlObject::m_vCameraIdList;

static std::map< CString, CString> mapMetaData;

static CString S_csTransGuid;

static BOOL S_bFirsConfigMsg = TRUE; 
...
CString GenerateGuid()
{
    ...
    if (ret_val == RPC_S_OK)
    {
        ...
        {
            csGuid = _T("GuidError");
            Trace(TRACE_ERROR, _T("Failed to generate GUID"));
        }
    }
    else
    {
...
/*
std::string ConvertToUNC(std::string sPath)
{
    WCHAR temp;
        ...
        &bufsize) == ERROR_MORE_DATA)
    {
        ...
        if (WNetGetUniversalName(sPath.c_str(),
            ...
            &bufsize) == NO_ERROR)
        {
            sRet = std::string(puni->lpUniversalName);
        }
        delete [] buf;
    }
    return sRet;
...
BOOL CGTOControlObject::InitializeGTOCO(CWnd& a_CWnd)
{
	BOOL bResult = TRUE;
	try
	{
...
bool CGTOControlObject::SendAlert(PSSBVALRESPONSE pRes)
{
	g_GTOCtrlObj.SendAlertMessage(pRes);
	return true;
}

bool CGTOControlObject::SendPLA(PSSBPLARESPONSE pRes)
{
	g_GTOCtrlObj.SendPLAMessage(pRes);
	return true;
}
/// <summary>
...
HRESULT CGTOControlObject::SetupConnection()
{
	...
	try
	{
		...
		if(lRC != ERROR_SUCCESS)
			throw CSSBCommException(SSB_COMM_REG_ERROR_OPEN_FAILED);
		lRC = ::RegQueryValueEx(hKey,
			   ...
			   &dwDataSize);
		if(lRC != ERROR_SUCCESS)
			throw CSSBCommException(SSB_COMM_REG_ERROR_QUERY_FAILED);			
		...
		hr = m_gPosConnection->SubscribeTo(_bstr_t(_T("CA,CB,C6,C7,C9")));
		if(hr != S_OK)
			throw _com_error(hr);
	...
	{
		hr = er.Error();
        Trace(TRACE_ERROR, _T("Unable to Set up Connection %s"), er.ErrorMessage());
		UnintializeGTOCO();
	}
	catch(CSSBCommException ex)
	{
		hr = 1L;
        Trace(TRACE_ERROR, _T("Unable to Set up Connection %s"), ex.GetErrorText());
		UnintializeGTOCO();
	}
	return hr;
}
...
HRESULT CGTOControlObject::SendImage(IMAGEDATA imaData)
{
    ...
    try
    {
        ...
        PLAMessage->SetValue(_T("height"), imaData.nImageHeight);
        PLAMessage->SetValue(_T("width"), imaData.nImageWidth);
        PLAMessage->SetValue(_T("length"), imaData.nImageLength);
        // PLAMessage->SetValue(_T("ImageType"), imaData.nImageType);
        PLAMessage->SetValue(_T("format"), ImageTypeToString(imaData.nImageType) );
        ...
        */
        ...
        if (!fOpts.Delete(imaData.cImgePath))
        {
...
const char * CGTOControlObject::ImageTypeToString(int nImageType)
{
    char * szImageFormat = "Unknown";
