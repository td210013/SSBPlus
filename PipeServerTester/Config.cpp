    ...
    if (S_pConfig == NULL)
    {
        ...
        if (S_pConfig != NULL)
        {
            ...
            else
            {
                ...
                // make sure you initialize COM in your App with CoInitialize.
...
CConfig::CConfig() : 
    csMessageSubscriptionList(szDEFAULT_MESSAGE_SUBSCRIPTION_LIST)
{
   Clear();
}
...
{
    xs.Deserialize(szXML_MESSAGE_SUBSCRIPTION_LIST, csMessageSubscriptionList );
    return eXsSuccess;
}
